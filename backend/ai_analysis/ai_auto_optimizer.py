...
class AIAutoOptimizer:
    ...
    def __init__(self, decision_module, interval=60):
        ...
        self.real_log = os.path.join(LOGS_DIR, 'real_trades.log')
    ...
    def analyze_and_adjust(self):
        ...
        # L'évolution ne se produit qu'en mode simulation
        if not self.decision_module.simulation_mode:
            return
        import random
        ...
        if self.gemini_analyzer and self.decision_module.simulation_mode:
            completed_log_path = os.path.join(LOGS_DIR, "completed_trades.log") # Assurez-vous que ce log est bien généré en mode simulation
    ...
    def _check_and_apply_rollback(self):
        ...
        if self.best_params and self.real_profit < self.best_profit * 0.5 and not self.freeze:
            ...
            self._log_decision(f"Chute de performance (profit actuel {self.real_profit:.4f} < 50% du meilleur profit {self.best_profit:.4f})", "rollback_to_best", old_params, new_params)
    ...
    def _run(self):
        ...
        while self.running:
            ...
            if self.decision_module and self.decision_module.simulation_mode:
                logger.info(f"[AI Optimizer] Bot fonctionnel en mode démo. Lancement du cycle d'évolution dans {self.interval} secondes.")
                time.sleep(self.interval) # Attend 10 minutes
